...
- 连接局域网里的另一台 server：用服务端机器的局域网 IP，在命令里写成 `SERVER_LAN_IP` 占位并替换成实际地址
...
```bash
cd /path/to/filefront-arena
npm run client:split -- --server http://SERVER_LAN_IP:31337 --name alice --team red
```

蓝队示例：

```bash
npm run client:split -- --server http://SERVER_LAN_IP:31337 --name bob --team blue
```

如果 macOS 第一次弹出 Terminal 自动化权限，请允许。若不想自动开窗口，也可以手动开三个终端：

```bash
npm run client -- --server http://SERVER_LAN_IP:31337 --name alice --team red --ui native
npm run client -- --server http://SERVER_LAN_IP:31337 --name alice-info --team red --ui monitor --watch
npm run client -- --server http://SERVER_LAN_IP:31337 --name alice-ops --team red --ui monitor --ops
```
